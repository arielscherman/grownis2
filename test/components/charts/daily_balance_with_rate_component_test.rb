...
class Charts::DailyBalanceWithRateComponentTest < ViewComponent::TestCase
  test "render the data attributes for the js controller" do
    depot = depots(:national_bank)
    render_inline(Charts::DailyBalanceWithRateComponent.new(depot, depot.daily_balances))
  ...
  test "renders the amounts data both in the main currency and its rate's" do
    ...
    result = render_inline(Charts::DailyBalanceWithRateComponent.new(depot, daily_balances))
